...
public partial class DualGridTilemap : TileMap {
    ...
    public void SetCell(int layer, Vector2I coords, Vector2I atlasCoords) {
        base.SetCell(layer, coords, 0, atlasCoords);
    ...
    public void EraseCell(int layer, Vector2I coords) {
        eraseDisplayTile(coords);
    }

    protected void eraseDisplayTile(Vector2 worldPos) {
        Vector2I cellPos = LocalToMap(worldPos);
        base.EraseCell(MAIN_LAYER, cellPos);
        for (int i = 0; i < NEIGHBOURS.Length; i++) {
            Vector2I newPos = cellPos + NEIGHBOURS[i];
            base.SetCell(DISPLAY_LAYER, newPos, 0, calculateTile(newPos));
        }
    }
